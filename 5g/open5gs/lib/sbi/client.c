...
static CURLcode sslctx_callback(CURL *curl, void *sslctx, void *userdata)
{
    ...
    /* Only set keylog callback if sslkeylog is configured */
    if (client->sslkeylog) {
    ...
    /*
     * Enforce TLS key exchange groups from env var.
     * This overrides OpenSSL 3.5.0's default of sending X25519MLKEM768
     * key_shares, ensuring the client ONLY offers the specified groups.
     */
    {
        const char *groups = getenv("OPEN5GS_TLS_GROUPS");
        if (groups) {
            if (SSL_CTX_set1_groups_list(ctx, groups) != 1) {
                ogs_warn("Client SSL_CTX_set1_groups_list(%s) failed", groups);
            }
        }
    }
#endif
        ...
        ogs_sbi_request_t *request, void *data)
{
    ...
    if (client->scheme == OpenAPI_uri_scheme_https) {
        ...
        } else if (getenv("OPEN5GS_TLS_GROUPS")) {
            /* Even without sslkeylog, we need the callback to enforce
             * TLS groups from the OPEN5GS_TLS_GROUPS env var */
            curl_easy_setopt(conn->easy, CURLOPT_SSL_CTX_FUNCTION,
                    sslctx_callback);
            curl_easy_setopt(conn->easy, CURLOPT_SSL_CTX_DATA, client);
        }
