        ...
        const char *sslkeylog_file)
{
     ...
     * This ensures the server ONLY accepts the specified groups,
     * overriding OpenSSL 3.5.0's defaults which include X25519MLKEM768.
     *
     * Set by toggle_pqc.sh via systemd Environment:
     *   PQC:       OPEN5GS_TLS_GROUPS=X25519MLKEM768:X25519
     *   Classical: OPEN5GS_TLS_GROUPS=X25519:P-256
     */
    {
        const char *groups = getenv("OPEN5GS_TLS_GROUPS");
        if (groups) {
            if (SSL_CTX_set1_groups_list(ssl_ctx, groups) != 1) {
                ogs_error("SSL_CTX_set1_groups_list(%s) failed: %s",
                        groups, ERR_error_string(ERR_get_error(), NULL));
                return NULL;
            }
        }
    }
