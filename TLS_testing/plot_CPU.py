...
with open(csv_file, 'r') as f:
    ...
    for row in reader:
        ...
        kem = row['KEM']
        try:
            cpu_val = float(row['CPU_ms'])
        except ValueError:
            cpu_val = 0.0
        
        if sig not in data:
            data[sig] = {}
        data[sig][kem] = cpu_val
...
output_file = 'benchmark_CPU_plot.png'
