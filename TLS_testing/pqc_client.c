/**
 * @file pqc_client.c
 * @brief A TLS client implementation using Post-Quantum Cryptography (PQC) for benchmarking.
 * 
 * This client uses OpenSSL 3.5.0's native PQC support to perform hybrid 
 * PQC handshakes. It measures handshake latency, CPU usage, and data 
 * throughput for comparison against classical cryptographic methods.
 * It defaults to using X25519MLKEM768 (hybrid X25519 + ML-KEM-768)
 * but can be configured to use other algorithms via command-line arguments.
 */
...
int main(int argc, char **argv) {
    ...
    char *ca_file    = NULL;
    char *cert_file  = NULL;  // Client certificate for mTLS
    char *key_file   = NULL;  // Client private key for mTLS
    char *groups     = "X25519MLKEM768";
    char *host_ip    = "127.0.0.1";
    
    // Parse command line arguments
    // -C: Path to CA certificate for server verification
    // -c: Path to client certificate (enables mTLS)
    // -k: Path to client private key  (enables mTLS)
    // -g: Specific key exchange groups (e.g., "X25519MLKEM768", "x25519")
    // -H: Server IP address (defaults to localhost)
    int opt;
    while ((opt = getopt(argc, argv, "C:c:k:g:H:")) != -1) {
        switch (opt) {
            case 'C':
                ca_file = optarg;
                break;
            ...
            case 'H':
                ...
                break;
            default:
                fprintf(stderr, "Usage: %s [-C ca_file] [-c client_cert] [-k client_key] [-g groups_list] [-H host_ip]\n", argv[0]);
                exit(EXIT_FAILURE);
        }
    }

    // Create a new SSL context for TLS client
    // OpenSSL 3.5.0 has native ML-KEM support — no provider loading needed
    ...
    // mTLS: load the client certificate and private key if provided
    if (cert_file) {
    ...
    if (key_file) {
        ...
        if (!SSL_CTX_check_private_key(ctx)) {
            fprintf(stderr, "Client key does not match the client certificate\n");
            ERR_print_errors_fp(stderr);
            exit(EXIT_FAILURE);
        }
    }

    // Set the requested PQC or hybrid key exchange groups
    // OpenSSL 3.5.0 native hybrid groups: X25519MLKEM768, SecP256r1MLKEM768
    ...
    if (SSL_connect(ssl) <= 0) {
        ...
        printf("PQC_METRIC_HANDSHAKE_MS: %.3f\n", hs_latency);
        printf("PQC_METRIC_USER_CPU_MS: %.3f\n", user_cpu);
        printf("PQC_METRIC_SYS_CPU_MS: %.3f\n", sys_cpu);

        // Check which group was actually negotiated (important for hybrid verification)
        ...
        printf("PQC_METRIC_THROUGHPUT_MBPS: %.3f\n", throughput_mbps);
    }
